...
namespace UnifyEditor_Setup.CloudAPI
{
    public class PowerShellHelper
    {
        ...
        /// 执行脚本,负载均衡主节点
        /// </summary>
        public static PSExcuteResult RunMasterPS(string localip, string ClusterPrimaryIP)
        {
            try
            {
                string PSPolicy = "Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process -Force;& ";
                string rootPath = AppDomain.CurrentDomain.BaseDirectory;
                string check_vm_ps = $"{rootPath}ps1\\Master.ps1 -localip '{localip}' -ClusterPrimaryIP '{ClusterPrimaryIP}'";
        ...
        /// 执行脚本,负载均衡备节点
        /// </summary>
        public static PSExcuteResult RunBackupPS(string localip, string ClusterPrimaryIP, string Pname,string Sname)
        {
            try
            {
                string PSPolicy = "Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process -Force;& ";
                string rootPath = AppDomain.CurrentDomain.BaseDirectory;
                string check_vm_ps = $"{rootPath}ps1\\Backup.ps1 -localip '{localip}' -ClusterPrimaryIP '{ClusterPrimaryIP}' -Pname '{Pname}' -Sname '{Sname}'";
                return RunScript(PSPolicy + check_vm_ps);
            }
            catch (Exception ex)
            {
