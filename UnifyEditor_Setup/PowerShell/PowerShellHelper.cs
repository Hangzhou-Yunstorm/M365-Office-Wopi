...
namespace UnifyEditor_Setup.CloudAPI
{
    public class PowerShellHelper
    {
        ...
        /// 执行脚本,创建场
        /// </summary>
        public static PSExcuteResult RunCreateFarmPS(string IntetnalURL, string ExternalUrl)
        {
            try
            {
                string PSPolicy = "Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process -Force;& ";
                string rootPath = AppDomain.CurrentDomain.BaseDirectory;
                string check_vm_ps = $"{rootPath}ps1\\CreateFarm.ps1 -IntetnalURL '{IntetnalURL}' -ExternalUrl '{ExternalUrl}' -root '{rootPath}'";
                return RunScript(PSPolicy + check_vm_ps);
            }
            catch (Exception ex)
            {
        ...
        /// 执行脚本,添加到场
        /// </summary>
        public static PSExcuteResult RunAddToFarmPS(string pcName)
        {
            try
            {
                string PSPolicy = "Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process -Force;& ";
                string rootPath = AppDomain.CurrentDomain.BaseDirectory;
                string check_vm_ps = $"{rootPath}ps1\\AddToFarm.ps1 -PCName '{pcName}' -root '{rootPath}'";
                return RunScript(PSPolicy + check_vm_ps);
            }
            catch (Exception ex)
            {
        ...
        public static PSExcuteResult Run2012R2IISPS()
        {
            try
            {
                ...
                return RunCMDScript(PSPolicy + check_vm_ps1);
            }
            catch (Exception ex)
            {
                return new PSExcuteResult() { STATE = "Error", DATA = "Exception：" + ex.Message };
